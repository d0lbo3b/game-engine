...
#define IO_READ_CHINK_SIZE 20971520
...
File io_file_read(const char *path) {
    ...
    while (true) {
        if (used + IO_READ_CHINK_SIZE + 1 > size) {
            size = used + IO_READ_CHINK_SIZE + 1;
        ...
        n = fread(data+used, 1, IO_READ_CHINK_SIZE, fp);
        if (n == 0) break;
...
int io_file_write(void *buffer, size_t size, const char *path) {
    ...
    if (!fp || ferror(fp)){
        ERROR_RETURN(1, "Could not write file: %s\n", path);
    }
