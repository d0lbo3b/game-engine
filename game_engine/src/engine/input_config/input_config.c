...
static char *config_get_value(const char *config_buffer, const char *value){
    ...
    if (!line){
        ERROR_EXIT("Could not find config value: %s."
                            "Consider deleting %s and restarting the program", value, CONFIG_PATH);
    }
...
static int config_load(void){
    File file_config = io_file_read(CONFIG_PATH);
...
void config_init(void){
    if (config_load() == 0) return;

    io_file_write((void*)CONFIG_DEFAULT, strlen(CONFIG_DEFAULT), CONFIG_PATH);
