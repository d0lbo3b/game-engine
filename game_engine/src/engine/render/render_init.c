...
SDL_Window *render_init_window(u32 width, u32 height){
    ...
    if (!window){
        ERROR_EXIT("Failed to init windows %s\n", SDL_GetError());
    }
...
void render_init_shaders(Render_State_Internal *state){
    state->shader_default = render_shader_create(SHADERS_FOLDER"default.vert", SHADERS_FOLDER"default.frag");
...
void render_init_color_texture(u32 *texture){
    ...
    u8 solid_White[4] = {0xFF, 0xFF, 0xFF, 0xFF};
